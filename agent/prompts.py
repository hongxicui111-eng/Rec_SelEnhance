...
# ═══════════════════════════════════════════════════════════════════════════════
# MLE_ANALYSIS_PROMPT - 核心分析Prompt 
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# STRUCTURE_OPTIMIZATION_PROMPT - 结构深度优化 
# ═══════════════════════════════════════════════════════════════════════════════
...
# ──────────────────────────────────────────────
# 自纠错 Prompt — 全部变更被验证拒绝后的重新提案
# ──────────────────────────────────────────────

REPROPOSAL_AFTER_REJECTION_PROMPT = """你之前提出的模型改进方案在验证/应用环节全部被拒绝，本轮迭代没有任何变更被成功应用。

这意味着用当前未修改的代码进行训练没有意义——我们需要重新提出 **更简单、更安全、更容易验证通过** 的改进方案。
...
1. **你之前的方案全部失败了**——不要重复类似的修改，必须提出 **截然不同** 的方案
2. **优先考虑参数修改**——参数修改不需要代码校验，更容易成功
3. **如果必须做结构修改，请做到最小化**——一次只修改一个函数/类，修改范围尽可能小
4. **SEARCH/REPLACE 的 search 文本必须与上面提供的实际源码逐字一致**
5. **不要提出与之前失败方案类似的修改**——否则同样会失败
...
{{
  ...
  "structural_changes": [
    {{
      "target_file": "源码文件",
      "target_class_or_function": "要修改的类或函数名",
      "description": "修改描述 — 必须是与之前完全不同的方案",
      "edits": [
        {{
          "search": "从实际源码中精确复制的代码片段",
          "replace": "修改后的代码片段"
  ...
  "explanation": "为什么这次方案更安全/更容易成功",
  "rationale": "与之前失败方案的区别说明"
}}
```

**⚠ 如果你不确定结构修改能否通过验证，优先只提出 param_changes — 参数修改不需要代码校验！**
"""
...
## 背景
LLM 分析了推荐模型的错误案例，给出了错误模式、模型瓶颈和改进建议。
...
## ⚠️ 关键约束 — 运行时基础设施
你的脚本执行时，以下代码会**自动注入在脚本开头**，你**不需要**自己定义这些变量和函数，也**绝对不能**重新定义它们（否则会覆盖正确的值）:

```python
# ── 运行时基础设施 (自动注入, 不要重复定义!) ──
import sys
sys.path.insert(0, "<模型目录路径>")  # 已自动设置，可直接 import 模型类
sys.path.insert(0, "<项目根目录>")     # 已自动设置
...
**使用方式：**
- 直接使用 `DATA_FILE` 变量加载数据: `_data = json.load(open(DATA_FILE))`
...
- 直接调用 `save_result(result)` 保存最终结果
- **不要**自行定义 `OUTPUT_FILE`、`DATA_FILE` 或 `save_result()`
- **不要**自行添加 `sys.path.insert` — 模型目录路径已自动注入
- 如果你需要的数据不在 _data 中, 用 None 表示

## ⚠️ 关于模型类导入
sys.path 已自动设置（包含模型目录和项目根目录），你可以**直接通过文件名导入模型类**，无需 copy 源码:

- 查看 model_info 中 model_file 和 modules_file 的路径，确定模型文件所在的目录名
- 如果模型目录名为 `Recmodel` 或 `Recmodel2` 等（且该目录有 `__init__.py`），使用: `from Recmodel.models import SASRec`
- 如果模型目录**没有** `__init__.py`（sys.path 已指向该目录），直接使用: `from models import SASRec`, `from modules import SelfAttention`
- **绝对不要**将模型类的源码 copy 到验证脚本中 — 使用 import 导入即可

## 重要约束
1. 代码必须是**独立的 Python 脚本**, 不依赖任何外部 API 或特殊库 (只用 numpy, json, collections, os, math, scipy.stats, torch)
2. 使用 `save_result()` 函数将结果写入 JSON 文件 (已在运行时基础设施中定义, 不要重复定义)
3. **⚠️ 绝对禁止模拟数据** — 不能使用 random、np.random、假设值来模拟结果。必须基于 DATA_FILE 中的真实数据计算。如果数据无法使用, 设置 success=false 并说明原因。
4. **⚠️ 结果必须有效** — 统计值不能是 0、NaN、None 或其他无效值。如果计算结果无效, 说明原因并设置 success=false。
5. JSON 输出格式必须包含以下字段:
```json
{{
  "hypothesis_id": "{hypothesis_id}",
...
# ── 新增: 假设易验证性筛选 Prompt (在验证前，先让 LLM 评估所有假设的数据获取难度并选出最易验证的) ──
HYPOTHESIS_EASE_SELECTION_PROMPT = """你是一位数据科学家，需要从多个可验证的假设中筛选出**最容易验证**的几个，作为先导试点验证。
...
## 任务
请对每个假设进行**数据获取难度评估**，然后选出最容易验证的假设。

### 评估维度
对于每个假设，你需要从以下维度评估其验证难度:
1. **数据就绪度**: 验证所需的数据是否已经在「已加载数据」或「数据盘点」中直接可用？
   - 所有数据都已就绪 → 简单
   - 需要少量补充获取 → 中等
   - 需要大量数据获取或模型内部数据 → 困难/极困难
2. **获取复杂度**: 缺失数据如果需要获取，获取路径有多复杂？
   - 直接从文件读取 → simple
   - 需要简单计算/聚合 → medium
   - 需要加载模型+推理 → complex
   - 需要注册 hook + 多步推理 + 提取中间结果 → very_complex
3. **验证步骤数**: 完整验证流程需要多少步骤？步骤越少越容易
4. **总体难度评级**: 综合以上维度，给出每个假设的总体验证难度
...
{{
  "difficulty_assessment": [
    {{
      "hypothesis_id": "假设ID",
      "claim_brief": "假设内容简述(20字以内)",
      "data_needed": ["这个假设需要的数据列表"],
      "data_already_available": ["已经就绪的数据列表"],
      "data_missing": ["需要获取的数据列表"],
      "acquisition_difficulty": "simple / medium / complex / very_complex",
      "total_verification_steps_estimate": 1,
      "overall_difficulty": "easy / moderate / hard / very_hard",
      "difficulty_reason": "简述为什么这个难度评级"
    }}
  ],
  "recommended_for_pilot": ["推荐的先导验证假设ID列表(按优先级排序)"],
  "reasoning": "选择这几个作为先导验证的理由(1-2句话)"
}}
```
...
#数据获取策略规划 Prompt (Step 1: 先规划，不直接生成代码) ──
...
## 获取策略 (分步蓝图)
以下策略规划包含多个步骤，合并为**一个连贯 Python 脚本**，步骤间通过变量传递数据:
...
## ⚠️ 数据样本 (请严格按此格式解析数据!)
以下是从项目数据文件中读取的真实样本, 展示了数据的实际格式 (分隔符、字段数、编码等):
**编写解析代码时必须参照这些样本**, 不要猜测分隔符或字段格式!
...
### 1. ⚠️ 优先使用已获取数据 (最重要!)
如果 `dependencies` 引用的数据已在「可用数据」或「数据盘点」中存在，**绝对不要从头重新获取!** 直接加载即可:
- `PREV_ACQUIRED_FILES` 中有路径 → `json.load(open(PREV_ACQUIRED_FILES["key"]))`  (key 就是数据盘点中显示的键名)
- 否则通过 `DATA_FILE` → `data = json.load(open(DATA_FILE)); value = data["key"]`  (key 见数据盘点中的「加载」说明)
- **注意**: 列表超过100项时, DATA_FILE 中的键名会变为 `key_sample` + `key_count` (不是原始 key); 字典超过2000条时会变为 `key` + `key_total`。请严格按数据盘点中的「加载」说明使用键名!

❌ 重新加载模型+推理获取评分 | ✅ 直接加载 PREV_ACQUIRED_FILES 中已有的评分数据

### 2. 运行时基础设施 (已自动注入, 禁止重复定义!)
以下常量/函数已在脚本开头注入: `DATA_FILE`, `OUTPUT_FILE`, `PREV_ACQUIRED_FILES{{}}`(前一步数据路径dict), `save_result(result_dict)`
直接使用即可，不要重新定义。

### 3. 单脚本连贯执行
所有步骤在同一脚本中顺序执行，步骤间通过 Python 变量传递中间结果。只在脚本最后调用 `save_result()` 输出最终结果。

### 4. 其他约束
- **禁止模拟数据** — 无法获取真实数据时设置 success=false 并说明原因
- 模型内部数据: 用 `torch.load()` 加 checkpoint, PyTorch hooks 提取中间层输出
- 不要 try/except — 让错误自然抛出
- 脚本开头须有完整 import
...
## ⚠️ 数据样本 (请严格按此格式解析数据!)
以下是从项目数据文件中读取的真实样本, 展示了数据的实际格式:
**修正解析代码时必须参照这些样本**, 不要猜测分隔符或字段格式!
...
3. 使用 save_result() 函数保存结果 (已在运行时基础设施中定义, 不要重复定义)
4. 如果数据无法获取，设置 success=false 并说明原因
5. 不要模拟数据 — 失败就标记失败
6. ⚠️ 如果错误是解析格式问题 (如 split 分隔符错误), 请对照数据样本修正分隔符!
...
1. 分析错误原因, 修正代码
2. 保持相同的数据加载和统计逻辑
3. 使用 save_result() 函数将结果写入 JSON 文件 (已在运行时基础设施中定义, 不要重复定义)
4. 数据文件路径由 DATA_FILE 常量指定 (已注入), 使用 json.load(open(DATA_FILE)) 加载
5. ⚠️ 加载数据时请使用上方「可用数据」中列出的 JSON 键名, 不要猜测键名! 列表>100项时键名为 `key_sample` + `key_count`; 字典>2000条时键名为 `key` + `key_total`
6. ⚠️ 如果脚本涉及模型 API, 请参考上方「模型信息」中的参数和维度
7. 让错误正常抛出 — 不要用 try/except 包裹，让异常自然传播以获取完整错误信息

只输出修正后的完整 Python 脚本代码 (不要解释文字, 不要 markdown 标记)"""



# ── 新增: Query-Based 脑本修正 Prompt (复用 CodeQueryTool, 与 core.py 一致) ──
# 核心设计: 与 core.py 的 QUERY_BASED_FIX 模式相同 — LLM 先查询出错代码, 再修正
# 输出格式: query 阶段用 JSON, script 阶段直接输出 Python 代码

QUERY_BASED_SCRIPT_FIX_PHASE1_PROMPT = """{context_title}执行失败，请修正代码。

**核心变化**: 你不再需要盲修脚本! 系统为你提供了一个**代码索引** (文件名 + 类/函数签名),
你可以**按需查询**模型源码来理解正确的 API、数据格式、类定义等。
这比只看截断的 model_info 更精确 — 你能看到**完整的源码**!
...
## ⚠️ 关键约束 — 运行时基础设施
{runtime_infrastructure_instructions}

## 任务

你需要修正这个脚本。你有两种选择:

### 选择 A: 提出代码查询 (推荐 — 先查出错涉及的模型 API 和类定义!)

如果你还不理解错误涉及的模型类、函数、数据格式, **强烈建议先查询**!
很多脚本错误是因为对模型 API 的理解不完整 (如不知道某个类的构造参数、方法的返回格式等)。

**⚠ 重要: 代码查询与代码编辑是两个完全不同的概念!**
- **代码查询** (queries): 用 `search_function`、`read_file` 等动作获取代码内容。args 中的 `name` 应是纯粹的标识符, 如 `"SASRec"` 或 `"SASRec.__init__"`, **不要加任何前缀!**
- 不要写 `"SEARCH: class SASRec"` 作为查询名称, 正确写法是 `"SASRec"` 或 `"SASRec.__init__"`。

常用查询:
- `read_file("models.py")` 查看整个模型文件
- `search_function("SASRec")` 查看模型类的定义和方法
- `search_function("SASRec.__init__")` 查看 __init__ 的参数 (了解维度要求)
- `search_function("finetune")` 查看推理方法 (了解输入输出格式)
- `get_region("modules.py", 起始行, 结束行)` 查看具体代码片段
- `search_pattern("ACT2FN")` 搜索某个变量/常量

输出格式:
```json
{{
  "phase": "query",
  "observation": "从错误中观察到的模式 (如: NameError 意味缺少 import, AttributeError 意味 API 误用)",
  "reasoning": "初步修正方向分析",
  "queries": [
    {{
      "action": "search_function",
  ...
  ⚠ **queries 必须是结构化对象数组, 不能是纯字符串!** 每个 query 必须包含 `action` 和 `args` 两个字段。

  "analysis_direction": "你打算如何修正这个脚本"
}}
```

### 选择 B: 直接输出修正后的脚本 (如果你已经从错误信息确认了修正方式)

{script_output_instructions}

**⚠ 关键提醒:**
- **选择 A (query) 是推荐的!** 如果错误涉及模型 API 使用 (如 AttributeError, TypeError, 维度不匹配), 先查询确认!
- **不要模拟数据** — 失败就标记 success=false 并说明原因
"""

QUERY_BASED_SCRIPT_FIX_PHASE2_PROMPT = """你之前提出的查询已经执行, 结果如下:
...
现在你拿到了具体的源码, 请继续分析并修正脚本。你仍然有两种选择:

### 选择 A: 继续查询 (如果需要更多上下文)
如果查询结果还不够理解错误, 你可以提出更多查询。输出格式同上一轮。

### 选择 B: 输出修正后的脚本 (如果你已经理解了错误)

{script_output_instructions}

**⚠ 关键**: 基于你查询到的源码, 确保脚本中使用的 API、类名、参数、数据格式与源码**完全一致**!
"""

QUERY_BASED_SCRIPT_FIX_FINAL_PROMPT = """你已经进行了多轮代码查询, 现在必须输出修正后的脚本!
...
**⚠ 关键**: 确保脚本中使用的 API 与你查询到的源码**完全一致**! 直接从查询结果中复制精确的类名、参数名、方法名!
"""

# ── 预定义的 context 配置 (数据获取 vs 验证) ──

DATA_ACQ_FIX_CONTEXT = {
    "context_title": "数据获取脚本",
    "runtime_infrastructure_instructions": """你的脚本执行时，以下代码会**自动注入在脚本开头**，你**不需要**自己定义，也**绝对不能**重新定义:
- `save_result()` 函数 — 将结果写入 JSON 文件
- `DATA_FILE` 常量 — 数据文件路径 (如果需要加载已有数据)
- `OUTPUT_FILE` 常量 — 输出文件路径
- `PREV_ACQUIRED_FILES` dict — 前一步已获取数据的 JSON 文件路径映射

数据加载规则:
- `PREV_ACQUIRED_FILES` 中有路径 → `json.load(open(PREV_ACQUIRED_FILES["key"]))` (key 见可用数据描述)
- 否则通过 `DATA_FILE` → `data = json.load(open(DATA_FILE)); value = data["key"]` (key 见可用数据描述)
- ⚠️ 列表超过100项时, DATA_FILE 中的键名会变为 `key_sample` (前50项样本) + `key_count` (总数)
- ⚠️ 字典超过2000条时, DATA_FILE 中的键名会变为 `key` (截断至2000) + `key_total` (总条数)

使用方式: 直接调用 `save_result(result)` 保存结果""",
    "script_output_instructions": """只输出修正后的完整 Python 脑本代码 (不要解释文字, 不要 markdown 标记)。
修正要求:
1. 保持相同的获取逻辑, 只修正导致错误的代码
2. 使用 save_result() 函数保存结果 (已在运行时基础设施中定义, 不要重复定义)
3. 如果数据无法获取, 设置 success=false 并说明原因
4. 不要模拟数据 — 失败就标记失败""",
}
...
VERIF_FIX_CONTEXT = {
    ...
    "runtime_infrastructure_instructions": """你的脚本执行时，以下代码会**自动注入在脚本开头**，你**不需要**自己定义，也**绝对不能**重新定义:
- `save_result()` 函数 — 将结果写入 JSON 文件
- `DATA_FILE` 常量 — 数据文件路径, 使用 `json.load(open(DATA_FILE))` 加载
- `OUTPUT_FILE` 常量 — 输出文件路径
- `PREV_ACQUIRED_FILES` dict — 前一步已获取数据的 JSON 文件路径映射 (可能为空 dict)
...
1. 保持相同的数据加载和统计逻辑
2. 使用 save_result() 函数将结果写入 JSON 文件 (已在运行时基础设施中定义, 不要重复定义)
3. 数据文件路径由 DATA_FILE 常量指定 (已注入), 使用 json.load(open(DATA_FILE)) 加载
4. 让错误正常抛出 — 不要用 try/except 包裹，让异常自然传播以获取完整错误信息""",
}
...
## ⚠️ 重要: 结果有效性检查
在分析之前, 你必须先检查结果是否有效:
1. **如果 success=false** → 验证未能成功执行, 判为 **UNVERIFIABLE** (无法查证), 原因写"验证执行失败"
2. **如果统计值为 0, NaN, None, inf, -inf** → 计算结果无效, 判为 **UNVERIFIABLE** (无法查证), 原因写"统计值无效, 验证未成功"
3. **如果缺少 statistics 字段** → 判为 **UNVERIFIABLE**
4. **只有统计值有效且非零** → 才能进行正常的假设判断

⚠️ 注意: 验证失败 ≠ 假设被否决! 验证脚本出错只是说明"我们无法查证假设"，不代表假设本身是错的。

## 判断标准
- **CONFIRMED**: 数据结果与"假设成立应观察到的现象"高度吻合
- **PARTIALLY_CONFIRMED**: 数据结果部分吻合, 但差异不够显著或存在混杂因素
- **REFUTED**: 数据结果与"假设不成立应观察到的现象"吻合, 或与假设方向相反
- **UNVERIFIABLE**: 验证未能成功执行, 或数据缺失, 或统计值无效 (0/NaN/None/inf), 无法得出有效结论

## 任务
根据验证结果, 给出你的判断。必须:
1. 首先检查结果有效性 (见上方"重要: 结果有效性检查")
2. 如果结果无效 → 直接判为 REFUTED 或 UNVERIFIABLE
3. 如果结果有效 → 解释数据结果的具体含义, 对比数据结果与期望现象, 给出明确判断
